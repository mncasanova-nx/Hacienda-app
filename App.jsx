...
function periodosCorral(corralId, movs) {
  var sorted = movs.filter(function(m){ return m.corral===corralId||m.corralDestino===corralId; })
    .sort(function(a,b){ return a.fecha>b.fecha?1:-1; });
  var stock=0, eventos=[];
  sorted.forEach(function(m){
    if((m.tipo==="entrada"||m.tipo==="paricion")&&m.corral===corralId) stock+=Number(m.cantidad);
    else if((m.tipo==="salida"||m.tipo==="muerte"||m.tipo==="traslado_externo")&&m.corral===corralId) stock-=Number(m.cantidad);
    else if(m.tipo==="mov_interno"||m.tipo==="traslado_campos"){
      if(m.corral===corralId) stock-=Number(m.cantidad);
      if(m.corralDestino===corralId) stock+=Number(m.cantidad);
    }
    eventos.push({fecha:m.fecha, stock:Math.max(0,stock)});
  });
  return eventos;
}

function calcularConsumoPromedio(corralId, movs, descargas, desde, hasta) {
  var eventos=periodosCorral(corralId,movs);
  if(eventos.length===0) return {kgTotales:0,animalesPromedio:0,promDiario:0,dias:0};
  var dias=diffDias(desde,hasta)||1;
  var animalesDiaAcum=0, diasContados=0;
  var puntos=[{fecha:desde,stock:0}].concat(eventos.map(function(e){return{fecha:e.fecha,stock:e.stock};})).concat([{fecha:hasta,stock:eventos[eventos.length-1].stock}]);
  for(var i=0;i<puntos.length-1;i++){
    var f1=puntos[i].fecha<desde?desde:puntos[i].fecha;
    var f2=puntos[i+1].fecha>hasta?hasta:puntos[i+1].fecha;
    if(f1>=f2) continue;
    var d=diffDias(f1,f2);
    animalesDiaAcum+=puntos[i].stock*d;
    diasContados+=d;
  }
  var animalesPromedio=diasContados>0?animalesDiaAcum/diasContados:0;
  var kgTotales=descargas.filter(function(d){ return d.corralId===corralId&&d.fecha>=desde&&d.fecha<=hasta; })
    .reduce(function(a,d){ return a+Number(d.kgTotales); },0);
  var promDiario=(animalesPromedio>0&&dias>0)?kgTotales/animalesPromedio/dias:0;
  return {kgTotales:kgTotales, animalesPromedio:Math.round(animalesPromedio), promDiario:promDiario, dias:dias};
}
...
export default function App() {
  ...
  var [corrales,     setCorrales]     = useState(CORRALES_INICIALES);
  var [movimientos,  setMovimientos]  = useState([]);
  var [descargas,    setDescargas]    = useState([]);
  var [suplementos,  setSuplementos]  = useState([]);
  var [lotesPastura, setLotesPastura] = useState({});
  var [campoFiltro,  setCampoFiltro]  = useState("el-prado");
  var [vista,        setVista]        = useState("registrar");
  var [loading,      setLoading]      = useState(true);
  var [toast,        setToast]        = useState(null);
  var [nuevoCorral,  setNuevoCorral]  = useState("");
  var [filtroCorral, setFiltroCorral] = useState("todos");
  var [filtroTipo,   setFiltroTipo]   = useState("todos");
  var [loteSeleccionado, setLoteSeleccionado] = useState(null);
  var [vistaCorral,  setVistaCorral]  = useState("prado-corral-1");

  var [form, setForm] = useState({
    fecha:todayStr(), tipo:"entrada", corral:"prado-corral-1", corralDestino:"prado-corral-2",
    campoPropietario:"el-prado", destinoExterno:"", categoria:CATEGORIAS[0], cantidad:"", observaciones:""
  });
  var [descForm, setDescForm] = useState({
    fecha:todayStr(), corralId:"prado-corral-1", tipoRacion:"Inicio", kgTotales:"",
    insumos:[{nombre:INSUMOS_DEFAULT[0],kg:""}]
  });
  var [suplForm, setSuplForm] = useState({
  ...
  var [pasturaForm, setPasturaForm] = useState({pastura:PASTURAS_DEFAULT[0].nombre,hectareas:"",rendimiento:PASTURAS_DEFAULT[0].rendimiento});
      ...
      .then(function(results){
        ...
        if(movs&&movs.length) setMovimientos(movs.map(function(m){
          return {id:m.id,fecha:m.fecha,tipo:m.tipo,corral:m.corral,corralDestino:m.corral_destino,
            ...
            categoria:m.categoria,cantidad:m.cantidad,observaciones:m.observaciones};
        }));
  ...
  var stock=calcularStock(corrales,movimientos);
  var historialLotes=calcularHistorialLotes(corrales,movimientos);
  var corralNombre=function(id){ var c=corrales.find(function(x){ return x.id===id; }); return c?c.nombre:id; };
  var corralesPropios=corrales.filter(function(c){ return c.campoId===campoFiltro&&c.tipo==="corral"; });
  var lotesCompartidos=corrales.filter(function(c){ return c.compartido; });
  var movFiltrados=movimientos.filter(function(m){
  ...
  var navItems=[["registrar","＋ Registrar"],["mixer","🚛 Mixer"],["suplementacion","🌿 Suplementación"],["historial","Historial"],["reporte","Reporte"],["lotes","Lotes"],["stock","Stock"]];
    ...
    <div style={{minHeight:"100vh",background:"#0f1117",color:"#e8e3d8",fontFamily:"Georgia,serif"}}>
      ...
      <div style={{maxWidth:920,margin:"0 auto",padding:"20px 14px"}}>
          ...
          <div style={{display:"grid",gap:18}}>
            <div style={S.card}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                ...
                <div><label style={S.label}>Categoría</label><select value={form.categoria} onChange={function(e){ setForm(function(f){ return {...f,categoria:e.target.value}; }); }} style={S.input}>{CATEGORIAS.map(function(c){ return <option key={c}>{c}</option>; })}</select></div>
                <div>
                  ...
                  <select value={form.corral} onChange={function(e){ setForm(function(f){ return {...f,corral:e.target.value}; }); }} style={S.input}>
                    {corralesPropios.length>0&&<optgroup label="Corrales">{corralesPropios.map(function(c){ return <option key={c.id} value={c.id}>{c.nombre}</option>; })}</optgroup>}
                    {(campoFiltro==="el-prado"||campoFiltro==="la-envidia")&&<optgroup label="Lotes compartidos">{lotesCompartidos.map(function(c){ return <option key={c.id} value={c.id}>{c.nombre}</option>; })}</optgroup>}
              ...
              {form.tipo==="mov_interno"&&(
                <div style={{marginTop:12}}>
                  ...
                  <select value={form.corralDestino} onChange={function(e){ setForm(function(f){ return {...f,corralDestino:e.target.value}; }); }} style={S.input}>
                    ...
                    {(campoFiltro==="el-prado"||campoFiltro==="la-envidia")&&lotesCompartidos.map(function(c){ return <option key={c.id} value={c.id}>{c.nombre}</option>; })}
                  </select>
                </div>
              )}
                ...
                <div style={{marginTop:12}}>
                  ...
                  <select value={form.corralDestino} onChange={function(e){ setForm(function(f){ return {...f,corralDestino:e.target.value}; }); }} style={S.input}>
                    {CAMPOS.filter(function(cp){ return cp.id!==campoFiltro; }).map(function(cp){
                      var cors=corrales.filter(function(c){ return c.campoId===cp.id||(c.compartido&&(cp.id==="el-prado"||cp.id==="la-envidia")); });
              ...
              {corrales.find(function(c){ return c.id===form.corral; })&&corrales.find(function(c){ return c.id===form.corral; }).compartido&&(
                <div style={{marginTop:12,background:"#1a1f2e",border:"1px solid #2a3048",borderRadius:8,padding:12}}>
                  <label style={{...S.label,color:"#60a5fa"}}>¿A qué campo pertenece esta hacienda?</label>
          ...
          <div style={{display:"grid",gap:18}}>
            ...
            <div style={S.card}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>
                ...
                <div>
                  <label style={S.label}>Tipo de ración</label>
                  <div style={{display:"flex",gap:5}}>
                    {TIPOS_RACION.map(function(r){
                      var col=r==="Inicio"?"#60a5fa":r==="Recría"?"#fbbf24":"#4ade80";
                ...
                {descForm.insumos.map(function(ins,i){
                  return (
                    <div key={i} style={{display:"grid",gridTemplateColumns:"1fr auto auto",gap:8,marginBottom:8,alignItems:"center"}}>
                      ...
                      <button onClick={function(){ setDescForm(function(f){ return {...f,insumos:f.insumos.filter(function(_,j){ return j!==i; })}; }); }} style={{background:"transparent",border:"1px solid #3a2a2a",color:"#f87171",borderRadius:6,padding:"7px 10px",cursor:"pointer"}}>✕</button>
                    </div>
                  );
                })}
              </div>
        ...
        {vista==="suplementacion"&&(
          <div style={{display:"grid",gap:18}}>
            <div style={S.card}>
              <div style={{fontSize:12,letterSpacing:2,color:"#8a7e6a",textTransform:"uppercase",marginBottom:16}}>Registrar Suplementación a Campo</div>
              ...
              {suplForm.kgTotales&&suplForm.animales&&Number(suplForm.animales)>0&&(
                <div style={{background:"#1a2a1a",border:"1px solid #2a4a2a",borderRadius:8,padding:12,marginBottom:12,fontSize:13,color:"#a8e8a8"}}>
                  Promedio: {(Number(suplForm.kgTotales)/Number(suplForm.animales)/(diffDias(suplForm.fechaDesde,suplForm.fechaHasta)||1)).toFixed(2)} kg/animal/día durante {diffDias(suplForm.fechaDesde,suplForm.fechaHasta)||1} días
                </div>
              )}
              ...
              <button onClick={guardarSuplemento} style={{background:"#1a3a1a",color:"#4ade80",border:"1px solid #2a6a2a",borderRadius:8,padding:"11px 0",cursor:"pointer",fontSize:14,fontWeight:700,width:"100%"}}>🌿 Registrar Suplementación</button>
            </div>
            <div style={S.card}>
                ...
                suplementos.map(function(sp){
                  ...
                  return (
                    <div key={sp.id} style={{background:"#1a2a1a",border:"1px solid #2a4a2a",borderRadius:10,padding:"12px 16px",marginBottom:10}}>
                      ...
                      <div style={{fontSize:12,color:"#8a7e6a",marginBottom:8}}>{fmtFecha(sp.fechaDesde)} → {fmtFecha(sp.fechaHasta)} ({sp.dias} días)</div>
                      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
                        ...
                        <div style={{textAlign:"center",background:"#0f1a0f",borderRadius:6,padding:8}}><div style={{fontSize:10,color:"#6a8a6a"}}>kg/animal/día</div><div style={{fontSize:15,fontWeight:700,color:"#fbbf24"}}>{Number(sp.kgAnimalDia).toFixed(2)}</div></div>
                      </div>
                    </div>
                  );
                })
                ...
                return (
                  <div key={m.id} style={{...S.card,padding:"13px 16px",display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,marginBottom:10}}>
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:14,color:"#e8dcc8",fontWeight:600}}>{m.cantidad} × {m.categoria}</div>
                    ...
                    <button onClick={function(){ eliminarMovimiento(m.id); }} style={{background:"transparent",border:"1px solid #3a2a2a",color:"#f87171",borderRadius:6,padding:"4px 9px",cursor:"pointer",fontSize:12}}>✕</button>
                  </div>
                );
              })
            }
          ...
          <div style={S.card}>
            ...
            {movFiltrados.length===0?<div style={{color:"#8a7e6a",textAlign:"center",padding:40}}>Sin movimientos</div>:
              movFiltrados.map(function(m,i){
                ...
                var cn=corralNombre;
                var txt="";
                if(m.tipo==="entrada") txt="El día "+fmtFecha(m.fecha)+", ingresaron "+m.cantidad+" "+m.categoria+" al "+cn(m.corral)+".";
                else if(m.tipo==="salida") txt="El día "+fmtFecha(m.fecha)+", salieron "+m.cantidad+" "+m.categoria+" del "+cn(m.corral)+".";
                else if(m.tipo==="mov_interno") txt="El día "+fmtFecha(m.fecha)+", se movieron internamente "+m.cantidad+" "+m.categoria+" del "+cn(m.corral)+" al "+cn(m.corralDestino)+" (mismo campo).";
                else if(m.tipo==="traslado_campos") txt="El día "+fmtFecha(m.fecha)+", se trasladaron "+m.cantidad+" "+m.categoria+" del "+cn(m.corral)+" al "+cn(m.corralDestino)+" (entre campos).";
                else if(m.tipo==="traslado_externo") txt="El día "+fmtFecha(m.fecha)+", salieron "+m.cantidad+" "+m.categoria+" del "+cn(m.corral)+" hacia "+(m.destinoExterno||"destino externo")+".";
                else if(m.tipo==="paricion") txt="El día "+fmtFecha(m.fecha)+", se registraron "+m.cantidad+" pariciones de "+m.categoria+" en el "+cn(m.corral)+".";
                else if(m.tipo==="muerte") txt="El día "+fmtFecha(m.fecha)+", fallecieron "+m.cantidad+" "+m.categoria+" en el "+cn(m.corral)+".";
                return (
        ...
        {vista==="lotes"&&(
          <div style={{display:"grid",gap:14}}>
            {historialLotes.length===0?<div style={{...S.card,textAlign:"center",color:"#8a7e6a",padding:50}}>Registrá entradas para ver el historial.</div>:
              historialLotes.filter(function(l){ var c=corrales.find(function(x){ return x.id===l.corralId; }); return c&&(c.campoId===campoFiltro||c.compartido); }).map(function(lote){
                ...
                var racionesDem={}, totalDemMS=0;
                Object.entries(lote.porCat).forEach(function(entry){ var cat=entry[0],cant=entry[1]; var consumo=CONSUMO_MS[cat]||8; var ms=cant*consumo*lote.dias; racionesDem[cat]={cant:cant,consumo:consumo,ms:ms}; totalDemMS+=ms; });
                  ...
                  <div key={lote.corralId} style={S.card}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12,flexWrap:"wrap",gap:8}}>
                      <div>
                        ...
                        <div style={{fontSize:12,color:"#8a7e6a",marginTop:2}}>{fmtFecha(lote.fechaEntrada)} → {lote.fechaSalida===todayStr()?"Actualmente":fmtFecha(lote.fechaSalida)} <span style={{marginLeft:8,color:"#b5925a",fontWeight:700}}>{lote.dias} días</span></div>
                      </div>
                    ...
                    {editando&&(
                      <div style={{background:"#1a1f2e",border:"1px solid #2a3048",borderRadius:10,padding:14,marginBottom:12}}>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
                          ...
                          <div><label style={S.label}>Hectáreas</label><input type="number" min="0" placeholder="ha" value={pasturaForm.hectareas} onChange={function(e){ setPasturaForm(function(f){ return {...f,hectareas:e.target.value}; }); }} style={S.input}/></div>
                    ...
                    {pastura&&!editando&&ofertaMS!==null&&(
                      <div style={{background:"#161a24",border:"1px solid #2a2f3e",borderRadius:8,padding:12,fontSize:13,color:"#c8c8b0",lineHeight:1.7}}>
                        El lote <strong style={{color:"#e8dcc8"}}>{lote.nombre}</strong> con <strong style={{color:"#a8e8a8"}}>{pastura.hectareas} ha</strong> de <strong style={{color:"#a8e8a8"}}>{pastura.pastura}</strong> ofreció <strong style={{color:"#fbbf24"}}>{fmtNum(ofertaMS)} kg MS</strong> — equivalente a <strong style={{color:"#fbbf24"}}>{diasRacion} raciones diarias</strong> para la carga registrada.
                        {balance<0&&<span style={{color:"#f87171"}}> ⚠ Déficit de {fmtNum(Math.abs(balance))} kg MS.</span>}
            ...
            <div style={{...S.card,marginBottom:14}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
                {CAMPOS.map(function(campo){
                  var ids=corrales.filter(function(c){ return c.campoId===campo.id; }).map(function(c){ return c.id; });
