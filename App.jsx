...
const CATEGORIAS = [
  "Vaquillonas","Donantes","Receptoras","Terneros","Terneras",
...
const CONSUMO_MS = {
  "Vaquillonas":8,"Donantes":12,"Receptoras":10,
...
const TIPOS_MOV = [
  ...
  {id:"correccion", label:"Corrección / Eliminar", icon:"⊘", color:"#fb923c"},
];
...
function calcularStock(corrales, movs) {
  ...
  corrales.forEach(function(c){
    stock[c.id]={};
    stockPorCampo[c.id]={};
    CATEGORIAS.forEach(function(cat){ stock[c.id][cat]=0; });
  });
  function sumar(corralId, campo, cat, n) {
    if(!stock[corralId]) return;
    stock[corralId][cat]=(stock[corralId][cat]||0)+n;
    if(campo){
      if(!stockPorCampo[corralId][campo]){ stockPorCampo[corralId][campo]={}; CATEGORIAS.forEach(function(c){ stockPorCampo[corralId][campo][c]=0; }); }
      stockPorCampo[corralId][campo][cat]=(stockPorCampo[corralId][campo][cat]||0)+n;
    }
  }
  movs.forEach(function(m){
    ...
    var campoOrigen=corralObj?(corralObj.compartido?(m.campoPropietario||null):corralObj.campoId):null;
    var campoDestino=corralDestinoObj?(corralDestinoObj.compartido?(m.campoPropietario||null):corralDestinoObj.campoId):null;
    if(m.tipo==="entrada"||m.tipo==="paricion"){
      sumar(m.corral,campoOrigen,m.categoria,+Number(m.cantidad));
    }
    else if(m.tipo==="salida"||m.tipo==="muerte"||m.tipo==="traslado_externo"||m.tipo==="correccion"){
      sumar(m.corral,campoOrigen,m.categoria,-Number(m.cantidad));
    }
    else if(m.tipo==="mov_interno"||m.tipo==="traslado_campos"){
      sumar(m.corral,campoOrigen,m.categoria,-Number(m.cantidad));
      if(m.corralDestino) sumar(m.corralDestino,campoDestino,m.categoria,+Number(m.cantidad));
    }
    else if(m.tipo==="cambio_categoria"){
      sumar(m.corral,campoOrigen,m.categoria,-Number(m.cantidad));
      if(m.categoriaDestino) sumar(m.corral,campoOrigen,m.categoriaDestino,+Number(m.cantidad));
    }
  });
  return {stock:stock,stockPorCampo:stockPorCampo};
}
...
function calcularHistorialLotes(corrales, movs) {
  ...
  corrales.forEach(function(c){
    ...
    var salidas=eventos.filter(function(e){ return e.tipo==="salida"||e.tipo==="muerte"||e.tipo==="correccion"; });
...
export default function App() {
  ...
  var guardarMovimiento = function() {
    ...
    SB.insert("movimientos",row).then(function(res){
      if(res){
        var localMov={id:id,fecha:form.fecha,tipo:form.tipo,corral:form.corral,
          corralDestino:form.corralDestino||null,campoPropietario:form.campoPropietario||null,
          destinoExterno:form.destinoExterno||null,categoria:form.categoria,
          categoriaDestino:form.tipo==="cambio_categoria"?form.categoriaDestino:null,
          cantidad:Number(form.cantidad),observaciones:form.observaciones||null};
        setMovimientos(function(l){ return [localMov].concat(l); });
        setForm(function(f){ return {...f,cantidad:"",observaciones:""}; });
  ...
  var exportarPDF = function() {
    ...
    // Resumen por campo (totales generales)
    var resumenCamposHTML = CAMPOS.map(function(campo) {
      ...
      movimientos.forEach(function(m){
        ...
        if(m.tipo==="entrada"||m.tipo==="paricion"){ if(esMiCampo) tot[m.categoria]=(tot[m.categoria]||0)+Number(m.cantidad); }
        else if(m.tipo==="salida"||m.tipo==="muerte"||m.tipo==="traslado_externo"||m.tipo==="correccion"){ if(esMiCampo) tot[m.categoria]=(tot[m.categoria]||0)-Number(m.cantidad); }
        else if(m.tipo==="mov_interno"||m.tipo==="traslado_campos"){
          if(esMiCampo) tot[m.categoria]=(tot[m.categoria]||0)-Number(m.cantidad);
          if(esMiDestino) tot[m.categoria]=(tot[m.categoria]||0)+Number(m.cantidad);
        }
        else if(m.tipo==="cambio_categoria"&&esMiCampo){
          tot[m.categoria]=(tot[m.categoria]||0)-Number(m.cantidad);
          if(m.categoriaDestino) tot[m.categoriaDestino]=(tot[m.categoriaDestino]||0)+Number(m.cantidad);
        }
      });
      var cats = CATEGORIAS.filter(function(c){ return tot[c]>0; });
      var total = cats.reduce(function(a,c){ return a+tot[c]; },0);
      if(total===0) return '<div class="campo-box"><div class="campo-titulo">'+campo.nombre+'</div><div class="sin-datos">Sin hacienda registrada</div></div>';
      return '<div class="campo-box"><div class="campo-titulo">'+campo.nombre+' <span class="campo-total">'+total+' animales</span></div>'+
        '<table><tr>'+cats.map(function(c){ return '<th>'+c+'</th>'; }).join("")+'</tr>'+
        '<tr>'+cats.map(function(c){ return '<td class="num">'+tot[c]+'</td>'; }).join("")+'</tr></table></div>';
    }).join("");

    // Detalle por corral/lote/encenada agrupado por campo
    var detalleCorralesHTML = CAMPOS.map(function(campo) {
      // Corrales propios del campo + compartidos con hacienda de este campo
      var corralesCampo = corrales.filter(function(c){ return c.campoId===campo.id && !c.compartido; });
      var corralesComp  = corrales.filter(function(c){ return c.compartido; });

      // Para cada corral calcular stock de ESTE campo usando stockPorCampo
      var filas = [];

      // Corrales propios
      corralesCampo.forEach(function(c){
        var s = stock[c.id]||{};
        var cats = CATEGORIAS.filter(function(cat){ return (s[cat]||0)>0; });
        var total = cats.reduce(function(a,cat){ return a+(s[cat]||0); },0);
        if(total===0) return;
        filas.push({nombre:c.nombre, tipo:c.tipo, cats:cats, s:s, total:total});
      });

      // Compartidos: solo mostrar si tienen hacienda de este campo
      corralesComp.forEach(function(c){
        var pc = (stockPorCampo[c.id]||{})[campo.id]||{};
        var cats = CATEGORIAS.filter(function(cat){ return (pc[cat]||0)>0; });
        var total = cats.reduce(function(a,cat){ return a+(pc[cat]||0); },0);
        if(total===0) return;
        filas.push({nombre:c.nombre+' (compartido)', tipo:c.tipo, cats:cats, s:pc, total:total});
      });

      if(filas.length===0) return '';

      var filasHTML = filas.map(function(f){
        var celdas = f.cats.map(function(cat){ return '<td>'+cat+': <b>'+f.s[cat]+'</b></td>'; }).join('');
        return '<tr><td class="corral-nombre">'+f.nombre+'</td><td class="corral-total">'+f.total+'</td>'+celdas+'</tr>';
      }).join('');

      return '<div class="detalle-campo"><div class="detalle-titulo">'+campo.nombre+' — detalle por corral / lote / encenada</div>'+
        '<table class="tbl-corrales"><tr><th>Ubicación</th><th>Total</th><th colspan="20">Categorías</th></tr>'+filasHTML+'</table></div>';
    }).join('');

    var movHTML = movimientos.slice(0,500).map(function(m){
      var t = TIPOS_MOV.find(function(x){ return x.id===m.tipo; })||TIPOS_MOV[0];
      return '<tr><td>'+m.fecha+'</td><td>'+t.label+'</td><td>'+m.cantidad+' x '+m.categoria+'</td><td>'+cn(m.corral)+(m.corralDestino?' &rarr; '+cn(m.corralDestino):'')+'</td><td>'+(m.observaciones||'')+'</td></tr>';
    }).join("");
    ...
    var reporteHTML = movimientos.slice(0,500).map(function(m){
      ...
      else if(m.tipo==="correccion") txt="El dia "+fmtFecha(m.fecha)+", corrección de stock: se eliminaron "+m.cantidad+" "+m.categoria+" del "+cn(m.corral)+(m.observaciones?" ("+m.observaciones+")":"")+".";
      return '<p>'+txt+'</p>';
    }).join("");
    ...
    win.document.write('<!DOCTYPE html><html><head><meta charset="utf-8"><title>HyH Ganadera - Reporte</title><style>'+
      ...
      'h2{font-size:16px;border-bottom:2px solid #b5925a;padding-bottom:6px;margin-top:36px;color:#7a5a2a;page-break-after:avoid;}'+
      ...
      '.campos-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:8px;}'+
      '.campo-box{border:1px solid #e0c88a;border-radius:8px;padding:14px;background:#fffdf7;}'+
      '.campo-titulo{font-size:13px;font-weight:bold;color:#7a5a2a;margin-bottom:8px;}'+
      '.campo-total{font-size:12px;color:#b5925a;font-weight:normal;margin-left:8px;}'+
      '.sin-datos{font-size:12px;color:#aaa;font-style:italic;}'+
      '.detalle-campo{margin-bottom:20px;page-break-inside:avoid;}'+
      '.detalle-titulo{font-size:13px;font-weight:bold;color:#5a3a10;background:#f5ede0;padding:8px 12px;border-radius:6px 6px 0 0;border:1px solid #e0c88a;border-bottom:none;}'+
      '.tbl-corrales{border-collapse:collapse;width:100%;font-size:12px;border:1px solid #e0c88a;}'+
      '.tbl-corrales th{background:#f5ede0;color:#7a5a2a;font-weight:bold;padding:6px 10px;text-align:left;border:1px solid #e0c88a;}'+
      '.tbl-corrales td{padding:5px 10px;border:1px solid #eee;}'+
      '.tbl-corrales tr:nth-child(even) td{background:#fafaf5;}'+
      '.corral-nombre{font-weight:600;color:#333;min-width:160px;}'+
      '.corral-total{font-weight:bold;color:#b5925a;text-align:center;}'+
      ...
      '<div class="header">'+
        ...
        '</div>'+
      '</div>'+
      '<h2>Stock Actual por Campo</h2>'+
      '<div class="campos-grid">'+resumenCamposHTML+'</div>'+
      '<h2>Detalle por Corral / Lote / Encenada</h2>'+detalleCorralesHTML+
  ...
  var _stockResult=calcularStock(corrales,movimientos); var stock=_stockResult.stock; var stockPorCampo=_stockResult.stockPorCampo;
            ...
            <div style={S.card}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                ...
                <div>
                  ...
                  <input type="number" min="1" placeholder="0" value={form.cantidad} onChange={function(e){ setForm(function(f){ return {...f,cantidad:e.target.value}; }); }} style={S.input}/>
                  {(function(){
                    var s=stock[form.corral]||{};
                    var disponible=s[form.categoria]||0;
                    var tiposQueDescuentan=["salida","muerte","traslado_externo","mov_interno","traslado_campos","cambio_categoria","correccion"];
                    if(tiposQueDescuentan.indexOf(form.tipo)===-1) return null;
                    return <div style={{marginTop:6,fontSize:12,color:disponible>0?"#8a9ab0":"#f87171"}}>
                      {disponible>0
                        ?<span>Disponibles en corral: <strong style={{color:"#e8dcc8"}}>{disponible} {form.categoria}</strong></span>
                        :<span>⚠ No hay <strong>{form.categoria}</strong> en este corral</span>}
                    </div>;
                  })()}
                </div>
              </div>
              {form.tipo==="mov_interno"&&(
                <div style={{marginTop:12}}>
                  ...
                  {(function(){
                    ...
                    var disponible=s[form.categoria]||0;
                    return disponible>0
                      ?<div style={{marginTop:6,fontSize:12,color:"#8a9ab0"}}>Disponibles en origen: <strong style={{color:"#e8dcc8"}}>{disponible} {form.categoria}</strong></div>
              ...
              {form.tipo==="traslado_externo"&&(
                <div style={{marginTop:12}}>
                  ...
                  {(function(){
                    ...
                    var disponible=s[form.categoria]||0;
                    return disponible>0
                      ?<div style={{marginTop:6,fontSize:12,color:"#8a9ab0"}}>Disponibles en corral: <strong style={{color:"#e8dcc8"}}>{disponible} {form.categoria}</strong></div>
                      :<div style={{marginTop:6,fontSize:12,color:"#f87171"}}>No hay {form.categoria} en este corral</div>;
                  })()}
                </div>
              )}
              {form.tipo==="muerte"&&(
                <div style={{marginTop:12,background:"#1a1a2e",border:"1px solid #3a2a4e",borderRadius:8,padding:12}}>
                  {(function(){
                    var s=stock[form.corral]||{};
                    var disponible=s[form.categoria]||0;
                    return disponible>0
                      ?<div style={{fontSize:12,color:"#8a9ab0"}}>Stock en corral: <strong style={{color:"#e8dcc8"}}>{disponible} {form.categoria}</strong></div>
                      :<div style={{fontSize:12,color:"#f87171"}}>No hay {form.categoria} en este corral</div>;
                  })()}
                </div>
              )}
              ...
              {corrales.find(function(c){ return c.id===form.corral; })&&corrales.find(function(c){ return c.id===form.corral; }).compartido&&(
                <div style={{marginTop:12,background:"#1a1f2e",border:"1px solid #2a3048",borderRadius:8,padding:12}}>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    {CAMPOS.map(function(cp){
                      var cid=cp.id;
                      return <button key={cid} onClick={function(){ setForm(function(f){ return {...f,campoPropietario:cid}; }); }} style={{background:form.campoPropietario===cid?"#1a2a4a":"#1e2233",color:form.campoPropietario===cid?"#60a5fa":"#8a7e6a",border:"1px solid "+(form.campoPropietario===cid?"#60a5fa":"#2a2f3e"),borderRadius:6,padding:"7px 18px",cursor:"pointer",fontSize:13,fontWeight:700}}>{cp.nombre}</button>;
                    })}
                  </div>
                </div>
              )}
          ...
          <div style={S.card}>
            ...
            {movFiltrados.length===0?<div style={{color:"#8a7e6a",textAlign:"center",padding:40}}>Sin movimientos</div>:
              movFiltrados.map(function(m,i){
                ...
                else if(m.tipo==="correccion") txt="El dia "+fmtFecha(m.fecha)+", se realizó una corrección de stock: se eliminaron "+m.cantidad+" "+m.categoria+" del "+cn(m.corral)+(m.observaciones?" ("+m.observaciones+")":"")+".";
                return (
          ...
          <div>
            ...
            <div style={{display:"grid",gap:10}}>
              {corrales.filter(function(c){ return c.campoId===campoFiltro||c.compartido; }).map(function(c){
                ...
                var porCampo=stockPorCampo[c.id]||{};
                var camposConStock=Object.keys(porCampo).filter(function(cp){ return CATEGORIAS.some(function(cat){ return (porCampo[cp][cat]||0)>0; }); });
                var esCompartido=c.compartido&&camposConStock.length>0;
                if(total===0) return null;
                return (
                  <div key={c.id} style={S.card}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                      <div style={{fontSize:14,fontWeight:700,color:"#e8dcc8"}}>{c.nombre}</div>
                      <div style={{fontSize:12,color:"#b5925a",fontWeight:700}}>{total} animales</div>
                    </div>
                    {esCompartido?(
                      <div style={{display:"flex",flexDirection:"column",gap:10}}>
                        {camposConStock.map(function(cp){
                          var sc=porCampo[cp]||{};
                          var catsC=CATEGORIAS.filter(function(cat){ return (sc[cat]||0)>0; });
                          var totalC=catsC.reduce(function(a,cat){ return a+(sc[cat]||0); },0);
                          var campoNombre=CAMPOS.find(function(x){ return x.id===cp; });
                          return (
                            <div key={cp} style={{background:"#1a1f2e",border:"1px solid #2a3a5e",borderRadius:8,padding:"10px 12px"}}>
                              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                                <div style={{fontSize:11,fontWeight:700,color:"#60a5fa",letterSpacing:1,textTransform:"uppercase"}}>{campoNombre?campoNombre.nombre:cp}</div>
                                <div style={{fontSize:11,color:"#b5925a",fontWeight:700}}>{totalC}</div>
                              </div>
                              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                                {catsC.map(function(cat){ return <div key={cat} style={{background:"#1e2233",border:"1px solid #2a2f3e",borderRadius:6,padding:"4px 10px",fontSize:12}}><span style={{color:"#8a9ab0"}}>{cat}</span><span style={{color:"#e8dcc8",fontWeight:700,marginLeft:6}}>{sc[cat]}</span></div>; })}
                              </div>
                            </div>
                          );
                        })}
